...

{
	...
	{
		if (init(bBuffer._pBuffer, bBuffer._ullCharsNumber, bBuffer._lBufferBlockNumberOnOverflow) != errNoError)
		{
...

{
	...
	if (_bsBufferStatus == TOOLS_BUFFER_INITIALIZED)
	{
		...
		_lBufferBlockNumberOnOverflow = TOOLS_DEFAULTBUFFERBLOCKNUMBERONOVERFLOW;
...

{
	...
	{
		if (init(pBuffer, strlen(pBuffer), _lBufferBlockNumberOnOverflow) != errNoError)
		{
			Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_BUFFER_INIT_FAILED);
		}
	}
	...
	{
		if (init((const char *)NULL, 0, _lBufferBlockNumberOnOverflow) != errNoError)
		{
			Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_BUFFER_INIT_FAILED);
		}
	}
...
#ifdef WIN32
Error Buffer::init(const char *pBuffer, __int64 llBufferLength, long lBufferBlockNumberOnOverflow)
#else
Error Buffer::init(const char *pBuffer, long long llBufferLength, long lBufferBlockNumberOnOverflow)
#endif

{

	if (pBuffer == (const char *)NULL || (llBufferLength < 0 && llBufferLength != -1) || lBufferBlockNumberOnOverflow <= 0)
	{
	...
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_OPERATION_NOTALLOWED, 1, (long)_bsBufferStatus);
	...
	if (llBufferLength == -1)
	{
		...
		if (allocMemoryBlockForBufferIfNecessary(ulBufferLength) != errNoError)
		{
			Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_BUFFER_ALLOCMEMORYBLOCKFORBUFFERIFNECESSARY_FAILED);
	...
	{
		if (allocMemoryBlockForBufferIfNecessary(llBufferLength) != errNoError)
		{
			Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_BUFFER_ALLOCMEMORYBLOCKFORBUFFERIFNECESSARY_FAILED);
...
#ifdef WIN32
Error Buffer::init(WCHAR *pwValue, long lBufferBlockNumberOnOverflow)

{
	...
	ulCharsLength = WideCharToMultiByte(CP_ACP, 0, pwValue, -1, (char *)NULL, 0, NULL, NULL);

	if ((pBuffer = new char[ulCharsLength]) == (char *)NULL)
	{
	...
	if (WideCharToMultiByte(CP_ACP, 0, pwValue, -1, pBuffer, ulCharsLength, NULL, NULL) == 0)
	{
	...
	if (init(pBuffer, strlen(pBuffer), lBufferBlockNumberOnOverflow) != errNoError)
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_BUFFER_INIT_FAILED);
...
Error Buffer::conversionFromCharToWCHAR(const char *pBuffer, WCHAR **pwValue, unsigned long *pulWCHARBufferLength)

{

	*pulWCHARBufferLength = MultiByteToWideChar(CP_ACP, 0, pBuffer, -1, (WCHAR *)NULL, 0);

	if ((*pwValue = new WCHAR[*pulWCHARBufferLength]) == (WCHAR *)NULL)
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_NEW_FAILED);
	...
	if (MultiByteToWideChar(CP_ACP, 0, pBuffer, -1, *pwValue, *pulWCHARBufferLength) == 0)
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_MULTIBYTETOWIDECHAR_FAILED);
...
#elif __QTCOMPILER__
#else
Error Buffer::conversionFromCharToWCHAR(const char *pBuffer, wchar_t *pwBuffer, unsigned long ulWCHARBufferLength)

{
	unsigned long ulLength;
	size_t ulReturn;

	if (pBuffer == (const char *)NULL || pwBuffer == (wchar_t *)NULL || ulWCHARBufferLength == 0)
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_ACTIVATION_WRONG);
	...
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_MBSTOWCS_FAILED);
...
Error Buffer::conversionFromWCHARToChar(const wchar_t *pwBuffer, char *pBuffer, unsigned long ulBufferLength)

{
	...
	if (pBuffer == (char *)NULL || pwBuffer == (const wchar_t *)NULL || ulBufferLength == 0)
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_ACTIVATION_WRONG);
	...
	if ((ulReturn = wcsrtombs(pBuffer, &pwBuffer, ulWCHARLength, &ps)) != ulWCHARLength)
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_WCSRTOMBS_FAILED);
...
Error Buffer::init(long lValue, long lBufferBlockNumberOnOverflow)

{
	...
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_SPRINTF_FAILED);

		return err;
	}

	if (init(pLongBuffer, strlen(pLongBuffer), lBufferBlockNumberOnOverflow) != errNoError)
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_BUFFER_INIT_FAILED);
...
Error Buffer::init(unsigned long ulValue, long lBufferBlockNumberOnOverflow)

{
	...
	if (init(pUnsignedLongBuffer, strlen(pUnsignedLongBuffer), lBufferBlockNumberOnOverflow) != errNoError)
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_BUFFER_INIT_FAILED);
...
Error Buffer::init(float fValue, long lBufferBlockNumberOnOverflow)

{
	...
	if (init(pFloatBuffer, strlen(pFloatBuffer), lBufferBlockNumberOnOverflow) != errNoError)
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_BUFFER_INIT_FAILED);
...
Error Buffer::init(double dValue, long lBufferBlockNumberOnOverflow)

{
	...
	if (init(pDoubleBuffer, strlen(pDoubleBuffer), lBufferBlockNumberOnOverflow) != errNoError)
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_BUFFER_INIT_FAILED);
...
#ifdef WIN32
Error Buffer::init(__int64 llValue, long lBufferBlockNumberOnOverflow)
#else
Error Buffer::init(long long llValue, long lBufferBlockNumberOnOverflow)
#endif

{
	...
	if (init(pLongLongBuffer, strlen(pLongLongBuffer), lBufferBlockNumberOnOverflow) != errNoError)
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_BUFFER_INIT_FAILED);
...
#ifndef WIN32
Error Buffer::init(unsigned long long ullValue, long lBufferBlockNumberOnOverflow)

{
	...
	if (init(pUnsignedLongLongBuffer, strlen(pUnsignedLongLongBuffer), lBufferBlockNumberOnOverflow) != errNoError)
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_BUFFER_INIT_FAILED);
...

{
	if (_bsBufferStatus != TOOLS_BUFFER_INITIALIZED)
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_OPERATION_NOTALLOWED, 1, _bsBufferStatus);
...

{
	...
	if (pBuffer == (const char *)NULL || pbStartWith == (Boolean_p)NULL)
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_ACTIVATION_WRONG);
	...
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_BUFFER_GETBUFFERLENGTH_FAILED);
...

{
	...
	if (pBuffer == (const char *)NULL || pbIsEqualWith == (Boolean_p)NULL)
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_ACTIVATION_WRONG);
	...
	else if (pBuffer == (const char *)NULL || (const char *)(*this) == (const char *)NULL)
		*pbIsEqualWith = false;
...
Error Buffer::isEqualWith(const Buffer &bBuffer, Boolean_p pbIsEqualWith) const

{

	if (_bsBufferStatus != TOOLS_BUFFER_INITIALIZED || bBuffer._bsBufferStatus != TOOLS_BUFFER_INITIALIZED)
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_OPERATION_NOTALLOWED, 1, _bsBufferStatus);
...

{
	...
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_BUFFER_APPEND_FAILED);
...

{
	...
	if (pBuffer == (const char *)NULL || (lBufferLength < 0 && lBufferLength != -1))
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_ACTIVATION_WRONG);
...

{
	...
	if (_bsBufferStatus == TOOLS_BUFFER_BUILDED || bBuffer._bsBufferStatus == TOOLS_BUFFER_BUILDED)
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_OPERATION_NOTALLOWED, 1, _bsBufferStatus);
...

{
	...
	else
	{
		...
		if ((pRemainingBuffer = new char[strlen(_pBuffer + lStartIndex) + 1]) == (char *)NULL)
		{
			Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_NEW_FAILED);
...

{
	...
	if (pInitial == (const char *)NULL || pFinal == (const char *)NULL)
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_ACTIVATION_WRONG);
	...
	while ((pDestinationToSubstituteNext = strstr(pDestinationToSubstituteNext, pInitial)) != (char *)NULL)
	{
		lSubstitutionNumber++;

		// update the previous and the next pointer to the header
		pDestinationToSubstitutePrev = pDestinationToSubstituteNext + lInitialLength;
	...
	lDifferentMemoryAfterSubstitution = (lFinalLength - lInitialLength) * lSubstitutionNumber;

	if (lDifferentMemoryAfterSubstitution > 0)
	{
		if (allocMemoryBlockForBufferIfNecessary(lDifferentMemoryAfterSubstitution) != errNoError)
		{
			Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_BUFFER_ALLOCMEMORYBLOCKFORBUFFERIFNECESSARY_FAILED);
	...
	while ((pDestinationToSubstituteNext = strstr(pDestinationToSubstituteNext, pInitial)) != (char *)NULL)
	{
		if (lDifferentMemoryAfterSubstitution == 0)
		{
			strncpy(pDestinationToSubstituteNext, pFinal, (unsigned int)lFinalLength);
		}
		else
		{
			// strdup alloc the memory with the malloc function
			pStringToCopy = strdup(pDestinationToSubstituteNext + lInitialLength);

			lTmpLength = strlen(pStringToCopy);
			strncpy(pDestinationToSubstituteNext + lFinalLength, pStringToCopy, (unsigned int)lTmpLength);
		...
		// update the previous and the next pointer to the header
		pDestinationToSubstitutePrev = pDestinationToSubstituteNext + strlen(pFinal);
		pDestinationToSubstituteNext = pDestinationToSubstitutePrev;
	}
...

{
	...
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_BUFFER_SUBSTITUTE_FAILED);
...
#ifdef WIN32
Error Buffer::substitute(const char *pInitial, __int64 ullFinal)
#else
Error Buffer::substitute(const char *pInitial, unsigned long long ullFinal)
#endif
...

{
	...
	for (ulIndex = 0; ulIndex < _ullCharsNumber; ulIndex++)
	{
		...
		{
			for (ulCopyIndex = ulIndex; ulCopyIndex < _ullCharsNumber; ulCopyIndex++)
			{
...
Error Buffer::truncateStartingFrom(StripType_t stStripType, char cCharToSearch)

{
	...
	{
		if ((pCharPosition = strchr(_pBuffer, cCharToSearch)) == (const char *)NULL)
		{
			Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_BUFFER_CHARNOTFOUND, 2, cCharToSearch, _pBuffer);
	...
	{
		if ((pCharPosition = strrchr(_pBuffer, cCharToSearch)) == (const char *)NULL)
		{
			Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_BUFFER_CHARNOTFOUND, 2, cCharToSearch, _pBuffer);
...

{
	...
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_BUFFER_INDEXOUTOFRANGE);
...

{
	...
	else
	{
		...
		{
			Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_BUFFER_SETBUFFER_FAILED);
...

{
	...
	if (lBufferLength == -1)
	{
		if (allocMemoryBlockForBufferIfNecessary(strlen(pBuffer)) != errNoError)
		{
			Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_BUFFER_ALLOCMEMORYBLOCKFORBUFFERIFNECESSARY_FAILED);
...
{
	Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_FILEIO_OPEN_FAILED, 1, pPathName);

	return err;
}

if (FileIO::seek(iFileDescriptor, (off_t)0, SEEK_END, &llFileLength) != errNoError)
{
	Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_FILEIO_SEEK_FAILED);
...
if (allocMemoryBlockForBufferIfNecessary(ullEndOffset - lStartOffset) != errNoError)
{
	Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_BUFFER_ALLOCMEMORYBLOCKFORBUFFERIFNECESSARY_FAILED);
...
if (FileIO::seek(iFileDescriptor, (off_t)lStartOffset, SEEK_SET, &llCurrentPosition) != errNoError)
{
	Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_FILEIO_SEEK_FAILED);
...
if (FileIO::readChars(iFileDescriptor, _pBuffer, ullEndOffset - lStartOffset, &llByteRead) != errNoError)
{
	Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_FILEIO_READCHARS_FAILED);
...
{
	Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_CLOSE_FAILED);
...
Error Buffer::strip(StripType_t stStripType, const char *pStringToStrip)

{
	...
	if (stStripType == STRIPTYPE_LEADING || stStripType == STRIPTYPE_BOTH)
	{
		...
		{
			if (ulBufferLength - lBufferIndex < ulStringToStripLength || strncmp(&(_pBuffer[lBufferIndex]), pStringToStrip, ulStringToStripLength))
				break;
		...
		if (lBufferIndex != 0)
		{
			...
			{
				_pBuffer[lBufferIndex - lFirstCharOfNewBufferIndex] = _pBuffer[lBufferIndex];
			}
	...
	{
		for (lBufferIndex = ulBufferLength - ulStringToStripLength; lBufferIndex >= 0;)
		{
			if (strncmp(&(_pBuffer[lBufferIndex]), pStringToStrip, ulStringToStripLength))
				break;
...
Error Buffer::strip(StripType_t stStripType, unsigned long ulCharactersNumberToStrip)

{
	...
	if ((stStripType == STRIPTYPE_BOTH && ulCharactersNumberToStrip * 2 > _ullCharsNumber) ||
		((stStripType == STRIPTYPE_LEADING || stStripType == STRIPTYPE_TRAILING) && ulCharactersNumberToStrip > _ullCharsNumber))
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_ACTIVATION_WRONG);
	...
	if (stStripType == STRIPTYPE_LEADING || stStripType == STRIPTYPE_BOTH)
	{
		for (lBufferIndex = ulCharactersNumberToStrip; lBufferIndex < _ullCharsNumber; lBufferIndex++)
		{
			_pBuffer[lBufferIndex - ulCharactersNumberToStrip] = _pBuffer[lBufferIndex];
		}
...
unsigned long Buffer::length(void) const { return (unsigned long)(*this); }
...

{
	...
	return (strcmp((const char *)(*this), (const char *)bBuffer) < 0 ? true : false);
}
...

{
	...
	else if (_bsBufferStatus == TOOLS_BUFFER_INITIALIZED && bBuffer._bsBufferStatus == TOOLS_BUFFER_BUILDED)
	{
	...
	else if (_bsBufferStatus == TOOLS_BUFFER_BUILDED && bBuffer._bsBufferStatus == TOOLS_BUFFER_INITIALIZED)
	{
...

{
	...
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_BUFFER_ISEQUALWITH_FAILED);
...

{
	...
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_BUFFER_GETCHAR_FAILED);
...

{
	...
	{
		Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_OPERATION_NOTALLOWED, 1, bBuffer._bsBufferStatus);
...
#ifdef WIN32
Error Buffer::allocMemoryBlockForBufferIfNecessary(__int64 ullBufferLengthToInsert)
#else
Error Buffer::allocMemoryBlockForBufferIfNecessary(unsigned long long ullBufferLengthToInsert)
#endif

{
	...
	if (_lBufferBlockNumber == 0 || lBufferLength + ullBufferLengthToInsert + 1 >= _lBufferBlockNumber * _lBufferBlockNumberOnOverflow)
	{
		...
		} while (lBufferLength + ullBufferLengthToInsert >= lLocalBufferBlockNumber * _lBufferBlockNumberOnOverflow);

		if ((pLocalStringBuffer = new char[lLocalBufferBlockNumber * _lBufferBlockNumberOnOverflow]) == (char *)NULL)
		{
			Error err = ToolsErrors(__FILE__, __LINE__, TOOLS_NEW_FAILED);
