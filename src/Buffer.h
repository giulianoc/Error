...

{
...
#ifdef WIN32
	Error allocMemoryBlockForBufferIfNecessary(__int64 ullBufferLengthToInsert);
#else
	Error allocMemoryBlockForBufferIfNecessary(unsigned long long ullBufferLengthToInsert);
#endif

  protected:
	char *_pBuffer;

	friend std::ostream &operator<<(std::ostream &osOutputStream, Buffer &bBuffer);
  ...
  public:
	...
	*/
...
#ifdef WIN32
	Error init(const char *pBuffer = "", __int64 llBufferLength = -1, long lBufferBlockNumberOnOverflow = TOOLS_DEFAULTBUFFERBLOCKNUMBERONOVERFLOW);
#else
	Error init(const char *pBuffer = "", long long llBufferLength = -1, long lBufferBlockNumberOnOverflow = TOOLS_DEFAULTBUFFERBLOCKNUMBERONOVERFLOW);
#endif

#ifdef WIN32
	Error init(WCHAR *pwValue, long lBufferBlockNumberOnOverflow = TOOLS_DEFAULTBUFFERBLOCKNUMBERONOVERFLOW);
	...
	/**
		...
		N.B.: If this method returns errNoError,
			...
			pointed by pwValue
	*/
	static Error conversionFromCharToWCHAR(const char *pBuffer, WCHAR **pwValue, unsigned long *pulWCHARBufferLength);
#elif __QTCOMPILER__
	// it seems wchar is not used yet by Android
#else
	static Error conversionFromCharToWCHAR(const char *pBuffer, wchar_t *pwBuffer, unsigned long ulWCHARBufferLength);

	static Error conversionFromWCHARToChar(const wchar_t *pwBuffer, char *pBuffer, unsigned long ulBufferLength);
#endif

	Error init(long lValue, long lBufferBlockNumberOnOverflow = TOOLS_DEFAULTBUFFERBLOCKNUMBERONOVERFLOW);

	Error init(unsigned long ulValue, long lBufferBlockNumberOnOverflow = TOOLS_DEFAULTBUFFERBLOCKNUMBERONOVERFLOW);

	Error init(float fValue, long lBufferBlockNumberOnOverflow = TOOLS_DEFAULTBUFFERBLOCKNUMBERONOVERFLOW);

	Error init(double dValue, long lBufferBlockNumberOnOverflow = TOOLS_DEFAULTBUFFERBLOCKNUMBERONOVERFLOW);

#ifdef WIN32
	Error init(__int64 llValue, long lBufferBlockNumberOnOverflow = TOOLS_DEFAULTBUFFERBLOCKNUMBERONOVERFLOW);
#else
	Error init(long long llValue, long lBufferBlockNumberOnOverflow = TOOLS_DEFAULTBUFFERBLOCKNUMBERONOVERFLOW);

	Error init(unsigned long long ullValue, long lBufferBlockNumberOnOverflow = TOOLS_DEFAULTBUFFERBLOCKNUMBERONOVERFLOW);
#endif
	...
	*/
	...
	Error isEqualWith(const char *pBuffer, Boolean_p pbIsEqualWith) const;

	Error isEqualWith(const Buffer &bBuffer, Boolean_p pbIsEqualWith) const;
	 ...
	 */
	Error append(float fValue, const char *pSprintfFormat = (const char *)NULL);

	/*
	 * pSprintfFormat could be something like: "%.3f"
	 */
	Error append(double dValue, const char *pSprintfFormat = (const char *)NULL);
	...
	*/
...
#else
	Error substitute(const char *pInitial, unsigned long long ullFinal);
#endif
	...
	Error truncateStartingFrom(StripType_t stStripType, char cCharToSearch);
	...
	Error setBuffer(float fValue, const char *pSprintfFormat = (const char *)NULL);

	Error setBuffer(double dValue, const char *pSprintfFormat = (const char *)NULL);
...
*/
	...
	Error strip(StripType_t stStripType = STRIPTYPE_LEADING, const char *pStringToStrip = " ");

	Error strip(StripType_t stStripType = STRIPTYPE_LEADING, unsigned long ulCharactersNumberToStrip = 1);
